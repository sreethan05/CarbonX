    ...
    <div className="pb-24 px-4 pt-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        <button 
          ...
          className="flex items-center justify-center p-2 rounded-xl bg-white border border-forest-100 text-carbon-600 hover:text-forest-800"
        >
      ...
      {/* Main Ledger Card */}
      <div className="bg-forest-900 text-white rounded-3xl p-6 shadow-xl mb-6 border border-forest-800 relative overflow-hidden">
        <div className="absolute right-0 top-0 w-32 h-32 bg-forest-700/20 rounded-full border border-forest-700/20 opacity-30 flex items-center justify-center pointer-events-none">
          <div className="w-20 h-20 rounded-full border border-forest-700/20" />
        </div>

        <div className="flex justify-between items-start mb-6 relative">
          <div>
            <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider">Carbon Ledger</span>
          ...
          <div className="bg-white/5 border border-white/10 p-2.5 rounded-xl text-emerald-400">
            <Shield className="w-5 h-5" />
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-4 text-xs relative">
          <div>
            ...
            <p className="text-[9px] text-emerald-400 mt-0.5">₹{(verifiedCredits * CREDIT_PRICE).toLocaleString('en-IN')}</p>
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-6 relative">
          <button
            ...
            disabled={balance <= 0}
            className="py-3 px-4 rounded-xl bg-emerald-500 text-white font-bold text-xs tracking-wider transition-all duration-300 hover:scale-[1.02] disabled:opacity-40 disabled:scale-100 disabled:cursor-not-allowed flex items-center justify-center gap-1.5"
          >
      ...
      <div className="bg-white rounded-2xl p-4 border border-forest-100 shadow-sm flex items-center justify-between text-xs mb-6">
        <div className="flex items-center gap-2.5">
          <div className="p-1.5 bg-forest-50 text-forest-700 rounded-lg">
            <CheckCircle className="w-4 h-4" />
          </div>
          <div>
            <p className="font-bold text-carbon-800">Linked UPI Account</p>
            <p className="text-[10px] text-carbon-400 font-mono">{upiId || 'No UPI linked'}</p>
          </div>
        </div>
        <span className="text-[9px] bg-forest-50 text-forest-700 font-bold px-2 py-0.5 rounded-full">
          VERIFIED
        </span>
      </div>
      ...
      <div className="flex items-center justify-between mb-4 px-1">
        ...
        <span className="text-[10px] text-carbon-400 flex items-center gap-1">
            ...
            <div key={act.id} className="flex justify-between items-start text-xs border-b border-forest-50 pb-3.5 last:border-0 last:pb-0">
              ...
              <div className="text-right">
                {act.amount !== undefined ? (
                  <p className={`font-black ${act.amount > 0 ? 'text-forest-700' : 'text-carbon-800'}`}>
        ...
        {isWithdrawOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white rounded-t-[32px] p-6 pb-8 border-t border-forest-100 shadow-2xl z-50 text-carbon-800"
            >
              ...
              <form onSubmit={handleWithdrawSubmit} className="space-y-5">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 px-2.5 py-1 bg-forest-700 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider hover:bg-forest-900 transition-all"
                    >
                      MAX
                    </button>
                  </div>
                ...
                <div className="bg-forest-50 p-4 rounded-2xl border border-forest-100 flex items-start gap-3">
                  ...
                  <div>
                    <h5 className="text-xs font-bold text-carbon-800">Smart Settlement</h5>
                    <p className="text-[10px] text-carbon-500 mt-0.5 leading-relaxed">
                      By submitting, you authorize the release of corresponding verified credits on CarbonX.
                    </p>
                  </div>
                </div>
                ...
                {success ? (
                  <motion.div 
                    ...
                    className="p-4 bg-forest-50 text-forest-700 rounded-2xl text-center font-bold text-xs border border-forest-200 flex items-center justify-center gap-2"
                  >
                    <CheckCircle className="w-5 h-5 text-forest-700" />
                    <span>Payout Successful! Check your bank SMS.</span>
                  </motion.div>
                ) : (
                  <button
                    type="submit"
                    disabled={withdrawing || !withdrawAmount || parseFloat(withdrawAmount) > balance}
                    className="w-full py-4 rounded-2xl bg-forest-800 text-white font-bold tracking-wider shadow-lg hover:bg-forest-900 transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {withdrawing ? (
                      <>
